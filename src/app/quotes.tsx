...
export default function HomeScreen() {
  const { status, signOut, student } = useAuth();
  ...
  useEffect(() => {
    let isActive = true;
    requestQuote()
      .then((nextQuote) => { if (isActive) setQuote(nextQuote); })
      .catch(() => {
        if (!isActive) return;
        setError('Could not reach the quote service. Here’s a quote to keep you inspired.');
        setQuote(fallbackQuotes[Math.floor(Math.random() * fallbackQuotes.length)]);
  ...
  if (status === 'restoring') {
    return (
      <SafeAreaView style={styles.safeArea}>
        <ActivityIndicator style={{ flex: 1 }} size="large" color="#078b9b" />
      </SafeAreaView>
    );
  }
    ...
    <ScrollView contentContainerStyle={styles.content}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginTop: 15 }}>
          <Text style={{ color: '#728098', fontSize: 12 }}>Signed in as {student?.name.split(' ')[0]}</Text>
          <Pressable accessibilityRole="button" onPress={() => router.replace('/')}>
            <Text style={{ color: '#078b9b', fontSize: 12, fontWeight: '700' }}>Profile</Text>
          </Pressable>
          <Pressable accessibilityRole="button" onPress={() => void signOut()}>
            <Text style={{ color: '#728098', fontSize: 12, fontWeight: '700' }}>Sign out</Text>
          </Pressable>
        </View>
      </View>
